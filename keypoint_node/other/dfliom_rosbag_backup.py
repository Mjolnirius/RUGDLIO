...
# Backup: this version worked through the dataset, but the output length didn't suit rviz to display. echo-ing the keypoints led to nonzero 
#   points, where as the current version only displays zeroes. this is due to the structure keeping function, which replaces dropped points 
#   with zeros, instead of deleting them. Explore: deleting dropped points and reduce the filesize, but how to keep structure the same for 
#   rviz?
...
class KeypointExtractor:
    ...
    def filter_pointcloud_by_indices(self, msg: PointCloud2, keep_indices: np.ndarray) -> PointCloud2:
        dtype_list = []
        ...
        if len(raw) % point_step != 0:
            raise ValueError(f"Invalid buffer size: {len(raw)} is not a multiple of point_step {point_step}")
        ...
        msg_out.row_step = msg.point_step * len(selected_points)
